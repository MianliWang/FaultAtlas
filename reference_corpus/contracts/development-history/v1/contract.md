...
Every manifest declaration is exactly one of two kinds. An **objective** declaration is compared with something outside the manifest -- the live `__all__`, the filesystem, the sealed vector files, the locked source documents, or the executor's own registries -- and the focused oracle fails if any objective leaf has no such consumer. A **descriptive** declaration has no independent source of truth; the exact leaf paths are enumerated in `descriptive_metadata.paths` (76 of them) and are never counted as verified assurance.
