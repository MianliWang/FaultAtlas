...
def test_no_p06_product_module_is_left_unaccounted() -> None:
    ...
    and the S01/S02/S03/S04 modules are excluded by exact name, because
...
def test_the_package_now_carries_twenty_four_production_modules() -> None:
    """The sealed twenty, and the S01/S02/S03/S04 modules added since.

    The decision's own two counts are sealed historical figures and stay at
    twenty. The checkout has exactly the four named P07 additions.
    """
    inventory = cast(dict[str, Any], _document()["product_inventory"])
...
def test_the_wheel_excludes_the_new_decision_and_keeps_twenty_four_modules(
...
def test_the_sdist_excludes_the_new_decision_and_keeps_twenty_four_modules(
    offline_distributions: tuple[pathlib.Path, pathlib.Path],
) -> None:
