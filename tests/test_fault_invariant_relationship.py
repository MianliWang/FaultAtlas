...
def test_bounded_s04_roadmap_and_current_gate() -> None:
    ...
    assert "`S1.P07.S04` is complete" in current
...
def distributions(tmp_path_factory: pytest.TempPathFactory) -> tuple[Path, Path]:
    output = tmp_path_factory.mktemp("invariant-relationship-distributions")
    env = os.environ | {
        "UV_OFFLINE": "1",
        "UV_NO_SYNC": "1",
...
def test_exact25_package_inventory_and_source_bytes(
    ...
    expected = sorted([*BASELINE_PRODUCTION, MODULE, PATTERN_COMPOSITION_MODULE])
